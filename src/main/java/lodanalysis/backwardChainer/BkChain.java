...
public class BkChain extends RuneableClass {
	...
	private void run(File datasetDir) {
...
//		System.out.println("invalids: " + invalids + ", valids: " + valids);
		// printSets();
	...
	private static void filterSubClassOfAndEquClass(Triple triple) {
		if (triple.getPredicate().equals(Rdfs.SUBCLASSOF) || triple.getPredicate().equals(Owl.EQUCLASS)) {
			classSet.add(triple.getSubject());
	...
	private static void filterSubPropertyOfAndEquProperty(Triple triple) {
		if (triple.getPredicate().equals(Rdfs.SUBPROPERTYOF) || triple.getPredicate().equals(Owl.EQUPROPERTY)) {
			propertySet.add(triple.getSubject());
