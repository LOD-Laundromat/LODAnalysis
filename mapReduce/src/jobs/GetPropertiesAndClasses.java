...
import mappers.GetPropertiesAndClassesMapper;
import reducers.GetPropertiesAndClassesReducer;
...
public class GetPropertiesAndClasses extends Configured implements Tool {

	private static Logger log = LoggerFactory.getLogger(GetPropertiesAndClasses.class);
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.out.println("Usage: GetPropertiesAndClasses <input dir> <output dir>");
			System.exit(0);
		}
		...
		int res = ToolRunner.run(new Configuration(), new GetPropertiesAndClasses(), args);
	...
	private Job createNewJob(String name) throws IOException {
		...
		job.setJarByClass(GetPropertiesAndClasses.class);
		job.setJobName(name);
	...
	public void sampleCommonResources(String[] args) throws Exception {
		Job job = createNewJob("Get All Properties And Classes");
		...
		// Job
		job.setMapperClass(GetPropertiesAndClassesMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(NullWritable.class);
		job.setReducerClass(GetPropertiesAndClassesReducer.class);
