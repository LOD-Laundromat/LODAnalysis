...
import mappers.GetAllPredicateClassesMapper;
import reducers.GetAllPredicateClassesReducer;
...
public class GetAllPredicateClasses extends Configured implements Tool {

	private static Logger log = LoggerFactory.getLogger(GetAllPredicateClasses.class);
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.out.println("Usage: GetAllPredicateClasses <input dir> <output dir>");
			System.exit(0);
		}
		...
		int res = ToolRunner.run(new Configuration(), new GetAllPredicateClasses(), args);
	...
	private Job createNewJob(String name) throws IOException {
		...
		job.setJarByClass(GetAllPredicateClasses.class);
		job.setJobName(name);
	...
	public void sampleCommonResources(String[] args) throws Exception {
		Job job = createNewJob("Get All Predicate Classes");
		...
		// Job
		job.setMapperClass(GetAllPredicateClassesMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(IntWritable.class);
		job.setReducerClass(GetAllPredicateClassesReducer.class);
